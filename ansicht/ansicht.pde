...
int r5 = int(random5); // random nr btw 0 and 255 for greyscale value / transparency
...
// beschikbare riso inktkleuren op ON in hex en in woorden
...
void setup(){
size(1784, 1240); // size for final print files
//size(874, 620);
noFill();
...
noLoop();
println(cname[r1] + ", " + cname[r2] + ", " + cname[r3] + " and " + cname[r4]);  // Print de kleuren van de inkt die nodig is
layerOne();
background(colours[11]);
layerTwo();
background(colours[11]);
...
output.println("LayerOne should be printed in " + cname[r1]); // write the ink colors to the file
